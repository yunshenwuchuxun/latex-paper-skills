        ...
        for raw in reader:
            rows.append(
                ExperimentRow(
                    ...
                    component_id=(raw.get("component_id") or "").strip(),
                    severity_levels=(raw.get("severity_levels") or "").strip(),
                    params=(raw.get("params") or "").strip(),
                    flops=(raw.get("flops") or "").strip(),
                    latency_ms=(raw.get("latency_ms") or "").strip(),
                )
            )
    return rows
...
def main() -> int:
    ...
    # Resolve config-relative paths from the experiments/ directory (not configs/),
    # so configs can reference siblings like ../paper and data/ consistently.
